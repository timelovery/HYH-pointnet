...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            loss1 = criterion(seg_pred_1, target, trans_feat, weights)
            loss2 = criterion(seg_pred_2, target, trans_feat, weights)
            loss = loss1 + loss2
            Con_loss = torch.nn.TripletMarginLoss(seg_pred_1, seg_pred_2)  # TODO:

            Con_loss.backward()
            loss.backward()
            optimizer.step()

            pred_choice = seg_pred_1.cpu().data.max(1)[1].numpy()
